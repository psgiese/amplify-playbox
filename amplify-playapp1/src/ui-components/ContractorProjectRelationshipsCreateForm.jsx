...
import { listContacts, listProjects } from "../graphql/queries";
import { createContractorProjectRelationships } from "../graphql/mutations";
...
export default function ContractorProjectRelationshipsCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setContact_id(initialValues.contact_id);
    setWork_start_date(initialValues.work_start_date);
    setWork_end_date(initialValues.work_end_date);
    setReview_id(initialValues.review_id);
    setMarked_complete_by(initialValues.marked_complete_by);
    setMarked_complete_on(initialValues.marked_complete_on);
    setWork_item_id(initialValues.work_item_id);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createContractorProjectRelationships.replaceAll(
              "__typename",
              ""
            ),
            variables: {
      ...
      {...getOverrideProps(
        overrides,
        "ContractorProjectRelationshipsCreateForm"
      )}
      {...rest}
    >
      <TextField
