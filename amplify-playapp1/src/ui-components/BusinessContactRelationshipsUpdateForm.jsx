...
import {
  getBusinessContactRelationships,
  getBusinesses,
  getContact,
  listBusinesses,
  listContacts,
} from "../graphql/queries";
import { updateBusinessContactRelationships } from "../graphql/mutations";
...
export default function BusinessContactRelationshipsUpdateForm(props) {
  const {
    id: idProp,
    businessContactRelationships: businessContactRelationshipsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = businessContactRelationshipsRecord
      ? {
          ...initialValues,
          ...businessContactRelationshipsRecord,
          businessesID,
          contactID,
        }
      : initialValues;
    setContact_role_name(cleanValues.contact_role_name);
    setRelationship_created_on(cleanValues.relationship_created_on);
    setIs_active_relationship(cleanValues.is_active_relationship);
    setRelationship_ended_on(cleanValues.relationship_ended_on);
    setRelationship_ended_by(cleanValues.relationship_ended_by);
    setBusinessesID(cleanValues.businessesID);
  ...
  const [
    businessContactRelationshipsRecord,
    setBusinessContactRelationshipsRecord,
  ] = React.useState(businessContactRelationshipsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getBusinessContactRelationships.replaceAll(
                "__typename",
                ""
              ),
              variables: { id: idProp },
            })
          )?.data?.getBusinessContactRelationships
        : businessContactRelationshipsModelProp;
      const businessesIDRecord = record ? record.businessesID : undefined;
      const businessesRecord = businessesIDRecord
        ? (
            await client.graphql({
              query: getBusinesses.replaceAll("__typename", ""),
              variables: { id: businessesIDRecord },
            })
          )?.data?.getBusinesses
        : undefined;
      setBusinessesID(businessesIDRecord);
      setSelectedBusinessesIDRecords([businessesRecord]);
      ...
      setBusinessContactRelationshipsRecord(record);
    };
    queryData();
  }, [idProp, businessContactRelationshipsModelProp]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          contact_role_name: contact_role_name ?? null,
          relationship_created_on: relationship_created_on ?? null,
          is_active_relationship,
          relationship_ended_on: relationship_ended_on ?? null,
          relationship_ended_by: relationship_ended_by ?? null,
          businessesID,
          contactID,
        };
        ...
        try {
          ...
          await client.graphql({
            ...
            variables: {
              input: {
                id: businessContactRelationshipsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "BusinessContactRelationshipsUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || businessContactRelationshipsModelProp)}
